...
func (g *Gemini) Generate(prompt string) string {
	resp, err := g.client.Models.GenerateContent(context.Background(), "gemini-1.5-flash", genai.Text(prompt), &genai.GenerateContentConfig{
		SystemInstruction: genai.Text("Create a tweet in a way that it is destined to go viral and structure it in a way that it will increase user interaction. Give only the tweet and nothing else ONLY TWEET!")[0],
